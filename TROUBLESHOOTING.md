...
### 7. Ошибка TypeError: object of type 'PosixPath' has no len()

**Описание:** Ошибка при создании суперпользователя с сообщением о PosixPath.

**Причина:** Django пытается использовать объект PosixPath вместо строки для имени базы данных.

**Решение:**
1. Проверить правильность .env файла:
```bash
cat .env | grep DB_
```

2. Убедиться что используется правильный движок БД:
```bash
# В .env должно быть:
...
3. Пересоздать .env файл:
```bash
rm .env
./setup.sh
```

### 8. Проблемы с паролями содержащими специальные символы

**Симптомы:**
- Ошибки при создании .env файла
- Неправильная интерпретация паролей

**Решение:**
- Используйте простые пароли без специальных символов для начала
- Или экранируйте специальные символы в .env файле одинарными кавычками

**Рекомендуемые символы для паролей:**
- Буквы: a-z, A-Z
- Цифры: 0-9  
